...
export function readLatestFile(path: string, extension: string): number[] {
	const animeNameFullPath = readFileSync(`${path}.${extension}`).toString();

	const splitted = animeNameFullPath.split("\n");
